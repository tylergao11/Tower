...
const scene=createCanvas(...m.scene.canvas),ctx=scene.getContext('2d');sprite(ctx,m.scene.intro.background,0,0,scene.width,scene.height,[0,0]);for(const [i,d]of m.scene.intro.dancers.entries())draw(ctx,'B05-dancer','dance',.15+i*.4,...d);draw(ctx,'L01','idle',.2,...m.scene.intro.lord);await fs.writeFile(path.join(out,'liubei-leisure.png'),scene.toBuffer('image/png'));
